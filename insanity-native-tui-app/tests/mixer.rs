...
impl SineSource {
    fn new(sr: u32, ch: u16, freq: f32) -> Self { Self { phase: 0.0, sr, ch, freq } }
}
...
fn mixer_sum_and_clip() {
    ...
    for s in out.iter() { assert!((*s - 1.0).abs() < 1e-5, "clipped {s}"); }
}
...
fn mixer_per_peer_volume() {
    ...
    for s in out.iter() { assert!((s - expected).abs() < 0.05, "vol50 {s}"); }
}
...
fn mixer_denoise_before_mix() {
    ...
    let noisy: Vec<f32> = (0..960).map(|i| if i%2==0 {0.5} else {-0.5}).collect();
...
fn mixer_master_volume() {
    ...
    mixer.handle_incoming(id, AudioChunk::new(0, AudioFormat::new(2,48000), vec![1.0;960]));
    mixer.set_master_volume(50);
    let mut out = vec![0f32; 10];
    mixer.fill_buffer(&mut out);
    let expected = 0.289;
    for s in out.iter() { assert!((s - expected).abs() < 0.06, "master50 {s}"); }
}
...
fn mixer_zero_peers_silence() {
    ...
    let mut out = vec![0f32; 960];
    mixer.fill_buffer(&mut out);
    for s in out.iter() { assert_eq!(*s, 0.0, "0 peers should be silence"); }
    // also after adding then removing, should return to silence
    let v = Arc::new(AtomicUsize::new(100));
    let d = Arc::new(AtomicBool::new(false));
    let id = uuid::Uuid::new_v4();
    mixer.add_peer(id, v, d, None);
    mixer.handle_incoming(id, AudioChunk::new(0, AudioFormat::new(2,48000), vec![0.5;960]));
...
fn mixer_many_peers_clipping() {
    ...
    for _ in 0..10 {
        ...
        let id = uuid::Uuid::new_v4();
        mixer.add_peer(id, v, d, None);
        mixer.handle_incoming(id, AudioChunk::new(0, AudioFormat::new(2,48000), vec![0.2;960]));
    }
    let mut out = vec![0f32; 960];
    mixer.fill_buffer(&mut out);
    for s in out.iter() {
...
fn mixer_volume_extremes() {
    ...
    mixer.handle_incoming(id0, AudioChunk::new(0, AudioFormat::new(2,48000), vec![1.0;960]));
    let mut out = vec![0f32; 10];
    mixer.fill_buffer(&mut out);
    for s in out.iter() { assert!(s.abs() < 1e-6, "vol0 {s}"); }
    ...
    mixer2.handle_incoming(id999, AudioChunk::new(0, AudioFormat::new(2,48000), vec![1.0;960]));
    let mut out2 = vec![0f32; 10];
...
fn opus_roundtrip_perceptual() {
    ...
    let chunk: Vec<f32> = (0..480).flat_map(|i| {
    ...
    assert!(insanity_core::loudness::calculate_loudness(&out) > 0.1, "decoded not silent");
