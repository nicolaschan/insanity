...
fn golden_two_peer_mix_perceptual() {
    let path = if Path::new("insanity-native-tui-app/testdata/golden/two_peer_mix.raw").exists() {
        Path::new("insanity-native-tui-app/testdata/golden/two_peer_mix.raw")
    } else {
        Path::new("testdata/golden/two_peer_mix.raw")
    };
    if !path.exists() {
        eprintln!("skip golden mix not generated");
        return;
    }
