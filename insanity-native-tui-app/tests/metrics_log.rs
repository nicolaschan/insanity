...
fn line_reports_all_counters_as_interval_deltas() {
    ...
    let line = format_metrics_line(&prev, &current, 1234, 0);
...
fn line_reports_peer_count_and_zero_delta() {
    ...
    let line = format_metrics_line(&snap, &snap, 0, 2);
...
fn line_saturates_on_counter_reset() {
    ...
    let line = format_metrics_line(&prev, &current, 0, 0);
    assert!(line.contains("gaps=0"), "{line}");
}
...
fn mixer_reports_peer_count_and_counters() {
    ...
    let line = format_metrics_line(&MixerMetrics::default(), &snap, 0, mixer.peer_count());
